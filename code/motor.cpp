...
#define RIGHT_MOTOR_PWM_FREQ_HZ   (2000U)

static uint32 right_motor_percent_to_duty(int duty_percent)
{
    if (duty_percent < 0)
    {
        duty_percent = 0;
...
void right_motor_init(int duty_percent)
{
    ...
    pwm_init(RIGHT_MOTOR_PWM_CHANNEL, RIGHT_MOTOR_PWM_FREQ_HZ,
        right_motor_percent_to_duty(duty_percent));
}
